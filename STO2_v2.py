...
class rsto2:
    ...
    def __init__(self, ifile, trim=None, rclip=None, verbose=False, tnorm=True, badpix=None):
        ...
        self.pos = [SkyCoord((np.float(self.hd1['UDP_RA'])+off2[0])*u.deg, (np.float(self.hd1['UDP_DEC'])+off3[0])*u.deg, frame='icrs'),
                    SkyCoord((np.float(self.hd1['UDP_RA'])+off2[1])*u.deg, (np.float(self.hd1['UDP_DEC'])+off3[1])*u.deg, frame='icrs'),
                    SkyCoord((np.float(self.hd1['UDP_RA'])+off2[2])*u.deg, (np.float(self.hd1['UDP_DEC'])+off3[2])*u.deg, frame='icrs')]
...
if __name__ == '__main__':
    ...
    #ifile = '/rdat/Projects/STO-2/Data/Level0/03279/SRC03279_04860.fits'
    #ifile = '/rdat/Projects/STO-2/Data/Level0.62/03559/OTF03559_00591.fits'
    #ifile = '/rdat/Projects/STO-2/Data/level0.62/03803/OTF03803_08250.fits'
    vv, spec, pos, hd1, dd1 = readSTO2(ifile, 0, retdd=True, rclip=2)
    print(pos)
    vv, spec, pos, hd1, dd1 = readSTO2(ifile, 1, retdd=True, rclip=2)
    print(pos)
    vv, spec, pos, hd1, dd1 = readSTO2(ifile, 2, retdd=True, rclip=2)
    print(pos)
    
    # example for Level 0.6 (line numbering has changed: [CII] is now lin = 2)
    #ifile = '/rdat/Projects/STO-2/Data/level0.6/03803/OTF03803_08246.fits'
    badpix2 = [481,482,483,484,485,486,487,488,489]    # bad pixels in cal scans
    
    badpix = None
    vv, spec, pos, hd1, dd1, rs = readSTO2(ifile, 2, retdd=True, retcl=True, rclip=2, badpix=badpix)
    print(spec.shape)
    print(vv[badpix2])
    print(spec[badpix2])
    ...
    cspec, report = cleanSpecPeak(spec,verbose=False, thresh=1E9, getReport=True, vv=vv, bflag=True, boff=1)
    nvv, ncspec = resample(vv, cspec, 2)
